...
import { useGetDiscuss, usePostDiscussChat } from '../../../../hooks/discuss/discuss';
...
function DiscussSection() {
  ...
  return (
    <Spin spinning={getDiscussLoad}>
      <div className="w-full min-h-[95vh] flex flex-col md:justify-between items-center md:py-5 relative">
        <div className="w-full">
          <div className="w-full flex items-center px-5 md:px-10 gap-5">
            ...
            <p className="text-[28px] font-bold text-dark">{getDiscussData?.data?.name}</p>
          </div>

          <Divider className='!mb-0' />
        </div>

        <div hidden={Boolean(!memoData?.length)} className="w-full h-[55vh] md:h-[70vh] overflow-y-auto space-y-10 p-5" id="chat-box">
          {memoData?.map((d: any) => {
            const isAI = isEqual(d?.sender, "nurovant")
            return (
            <div className='w-full flex gap-3'>
              <div className='w-[10%] md:w-[5%]'>
                <Image preview={false} hidden={!isAI} alt='nurovant' src={icon} />
              </div>
              <div className='max-w-[80%] md:max-w-[70%] space-y-3'>
                <p hidden={!isAI} className='p-3 bg-[#F9F9F9] rounded-2xl !rounded-bl-none text-sm font-medium text-dark'>{d?.message}</p>
                <p hidden={isAI} className='p-3 bg-primary rounded-2xl text-sm font-medium text-white'>
                  <p className='flex justify-between items-center gap-3'>
                    <span>{d?.message || d?.question}</span>
                    {(d?.question && postDisChatLoad) && <BiLoader className='animate-spin' />}
                    {(d?.question && postDisChatErr) && <BiErrorCircle className='text-red-500' />}
                  </p>
                </p>
        ...
        <div hidden={Boolean(memoData?.length)} className="w-full h-full">
          <div className="w-full h-full flex flex-col justify-center items-center gap-3 text-center">
            ...
            <div className='flex justify-center items-center gap-3'>
              {["What's this material about?", "Summary of this material", "Keywords regarding this material"].map(question => (
                <Button
        ...
        <Form form={form} onFinish={handleSubmit} className='!w-full px-5'>
          <Form.Item name="question">
            <Input required size='large' placeholder='Type a message' className='!h-[50px] !rounded-2xl' />
          </Form.Item>
          <Button
