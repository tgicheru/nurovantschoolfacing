...
import { useDeleteSpeechRate, useGetSpeechRates, usePostSpeechRate } from '../../../hooks/speechrate/speechrate'
import RecordSection from '../questiontracker/sections/record'
...
function SpeechRate() {
  ...
  const {
    data: getSpeechRatesData,
    refetch: getSpeechRateFetch,
    isLoading: getSpeechRatesLoad,
  } = useGetSpeechRates({}, (res: any) => setIsRecOpen(res && !res?.is_voice_recorded))

  const {
    mutate: postSpeechRateAction,
    isLoading: postSpeechRateLoad,
  } = usePostSpeechRate(() => {
    getSpeechRateFetch()
    form.resetFields()
    setPayload({})
  ...
  const {
    mutate: deleteSpeechRateAction,
    isLoading: deleteSpeechRateLoad,
  } = useDeleteSpeechRate(getSpeechRateFetch)

  const handleSubmit = (data: any) => postSpeechRateAction({ ...data, ...payload, feature_type: "speech_rate" })
  ...
  return (
    <div className='w-full py-5 space-y-5'>
      ...
      <BorderHOC loading={getSpeechRatesLoad} rounded='rounded-xl' className='w-full h-full'>
        <div className='w-full h-full p-5 space-y-2'>
          <div className='w-full h-full flex flex-col md:flex-row justify-between items-center gap-5'>
            <div className='flex items-center gap-2'>
              <p className='text-2xl font-bold text-[#161617]'>{getSpeechRatesData?.data?.length}</p>
          ...
          <EmptyDisplay hidden={getSpeechRatesData?.data?.length} className='w-full h-[50vh]'>
            <Button onClick={onOpen} className='!text-sm !font-bold bg-[#4970FC]' icon={<LuPlus className='text-xl' />} size='large' type='primary' shape='round'>Check your Speech Rate</Button>
          </EmptyDisplay>

          <div hidden={!getSpeechRatesData?.data?.length} className='w-full'>
            <div hidden={!isEqual(list, "grid")} className='w-full h-full'>
              <div className='w-full grid sm:grid-cols-2 md:grid-cols-3 gap-5'>
                {getSpeechRatesData?.data?.map((d: any) => {
                  const isReady = isEqual(d?.status?.toLowerCase(), "success")
                  const onView = () => (isReady ? handleView(d?._id) : {}) 
                  const onDelete = (e: any) => {e?.stopPropagation?.(); deleteSpeechRateAction(d?._id)} 
                  return (
                    <BorderHOC rounded='rounded-xl' className='w-full h-full' childClass='w-full h-full p-3 space-y-3'>
                      <div className='flex justify-between items-center gap-5'>
                        ...
                        <Dropdown menu={{ items: [
                          ...
                          <Button onClick={handleOption} loading={deleteSpeechRateLoad} type='text' icon={<PiDotsThreeOutline className='text-2xl' />} />
                        </Dropdown>
                      </div>
      ...
      >
        <div className='w-full p-5 space-y-5 bg-white rounded-3xl'>
          ...
          <Form layout='vertical' onFinish={handleSubmit} form={form}>
            ...
            <Button loading={postSpeechRateLoad} className="bg-[#4970FC]" block size="large" type="primary" htmlType="submit" shape="round">Check your Speech Rate</Button>
          </Form>
        </div>
      </Drawer>
      ...
      <Modal
        ...
        onCancel={onRecClose}
      >
        <RecordSection successAction={getSpeechRateFetch} />
      </Modal>
    </div>
  )
}
